...
let tags = {
  ...
  try {
    ...
    let status = await conn.fetchStatus(m.sender).catch(() => null);
   ...
   let bjir = 'https://telegra.ph/file/b6756bcee9059b0c532e7.jpg'
    ...
    // Membuat grup berdasarkan kategori (tags)
    let groups = {};
    for (let tag in tags) {
        groups[tag] = [];
    ...
    let replace = {
      ...
      ucpn,platform, wib, mode, _p, eris, age, tag, name, prems, level, limit, status, weton, week, date, dateIslamic, time, totalreg, rtotalreg, role,
      readmore: readMore
    }
    text = text.replace(new RegExp(`%(${Object.keys(replace).sort((a, b) => b.length - a.length).join`|`})`, 'g'), (_, name) => '' + replace[name])

 let fkon = { key: { fromMe: false, participant: `${m.sender.split`@`[0]}@s.whatsapp.net`, ...(m.chat ? { remoteJid: '16504228206@s.whatsapp.net' } : {}) }, message: { contactMessage: { displayName: `${name}`, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:;a,;;;\nFN:${name}\nihitam1.TEL;waid=${m.sender.split('@')[0]}:${m.sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`}}}
